    ...
    try {
      switch (event.type) {
        ...
        case "customer.subscription.updated": {
          ...
          const tier = sub.metadata.tier as "PRO" | "PREMIUM" | "PLUS";
          const role = sub.metadata.role as "BARBER" | "CLIENT";
          ...
          if (userId && tier) {
            ...
            if (role === "BARBER") {
              await prisma.barberSubscription.upsert({
                where: { userId },
                create: {
                  userId,
        ...
        case "customer.subscription.deleted": {
          ...
          if (role === "BARBER") {
            await prisma.barberSubscription.updateMany({
...
export async function createSubscriptionCheckout(params: {
  ...
  const priceMap: Record<string, string> = {
    "BARBER_PRO": config.STRIPE_PRICE_BARBER_PRO,
    "BARBER_PREMIUM": config.STRIPE_PRICE_BARBER_PREMIUM,
    "CLIENT_PLUS": config.STRIPE_PRICE_CLIENT_PLUS,
  };
